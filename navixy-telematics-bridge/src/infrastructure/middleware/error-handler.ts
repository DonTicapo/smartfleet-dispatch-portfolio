...
import { DomainError, EntityNotFoundError, ValidationError, DuplicateEntityError, NavixyApiError } from '../../domain/errors/domain-error.js';
...
export function registerErrorHandler(app: FastifyInstance): void {
  app.setErrorHandler((error, _request, reply) => {
    ...
    if (error instanceof NavixyApiError) {
      reply.code(502).send({ error: error.code, message: error.message });
      return;
    }

    if (error instanceof ZodError) {
      reply.code(400).send({ error: 'VALIDATION_ERROR', message: error.errors });
      return;
    }
